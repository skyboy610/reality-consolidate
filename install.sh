...
drop_dup_from_tune() {
    local errfile="$1" dirs d changed=0
    [ -f "$NGINX_TUNE" ] || return 1
    dirs=$(grep -oE '"[a-z_]+" directive is duplicate' "$errfile" 2>/dev/null | grep -oE '"[a-z_]+"' | tr -d '"' | sort -u)
    [ -n "$dirs" ] || return 1
    for d in $dirs; do
        if grep -qE "^[[:space:]]*${d}([[:space:]]|;)" "$NGINX_TUNE"; then
            sed -i -E "/^[[:space:]]*${d}([[:space:]]|;)/d" "$NGINX_TUNE"
            info "Removed duplicate directive '${d}' from the tuning file"
            changed=1
        fi
        if grep -qE "^[[:space:]]*${d}([[:space:]]|;)" "$NGINX_CONF"; then
            sed -i -E "/^[[:space:]]*${d}([[:space:]]|;)/d" "$NGINX_CONF"
            info "Removed duplicate directive '${d}' from nginx.conf"
            changed=1
        fi
    done
...
apply_nginx() {
    local backup_dir="${1:-}" try
    for ((try = 0; try < 6; try++)); do
        nginx -t 2>/tmp/r443.nginx.err && break
        if grep -q 'is already loaded' /tmp/r443.nginx.err && drop_duplicate_load_module; then
            continue
        fi
        if grep -q 'directive is duplicate' /tmp/r443.nginx.err && drop_dup_from_tune /tmp/r443.nginx.err; then
            continue
        fi
        break
    done
